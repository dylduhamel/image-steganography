...
int main(int argc, char *argv[]) {
    ...
    argsOk = processImageArgs(argc, argv, &bit, imageFilename); /* bit as pointer because we want to change it in program */
    if(!argsOk) 
        return 1;
...
int recoverMessage(struct Image *img, char bits[]) {
    ...
    /* assigns the appropriate mask, shift length, and loop length for specified bite length from args */
    if(strcmp(bits, "1") == 0) {
        mask = 0x01;
        shift = 1;
        loop = 6;
